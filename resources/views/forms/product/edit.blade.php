...
<!-- @if ($errors->any())
	...
	<section class="create-product">
		...
		<form method="post" action="/product/{{ $product->id.'/edit' }}" id="productform" enctype="multipart/form-data" class="form">
			@csrf
	...
	comment: https://stackoverflow.com/a/8524863 -->
			<div class="form__field form__field--inline-block form__field--w50 form__field--va-t">
				...
				<input class="form__input form__input--w100" type="text" id="name" name="name" value="{{ $product->name }}" placeholder="My Product" required>
			</div>
			<!-- Categories as tags -->
			<div class="form__field form__field--inline-block form__field--w50 form__field--mt-0">
			...
			<div class="form__field">
				...
				<textarea class="form__textarea" id="description" name="description">{{ $product->description }}</textarea>
			</div>
			...
			<div class="form__field form__field--w33 form__field--inline-block">
				...
				<select class="form__select" name="listed" id="listed" form="productform">
					<option class="form__option" value="1" {{ $product->listed ? 'selected' : '' }}>
						Listed
					</option>
					<option class="form__option" value="0" {{ !$product->listed ? 'selected' : '' }}>
						Unlisted
					</option>
				</select>
			</div>
			<div class="form__field form__field--w33 form__field--inline-block">
				...
				<select class="form__select" name="available" id="available" form="productform">
					<option class="form__option" value="1" {{ $product->available ? 'selected' : '' }}>
						Yes
					</option>
					<option class="form__option" value="0" {{ !$product->available ? 'selected' : '' }}>
						No
					</option>
				</select>
			</div>
			<div class="form__field form__field--w33 form__field--inline-block">
				...
				<input type="number" class="form__input form__input--w100" name="days_to_loan" step="1" value="{{ $product->days_to_loan }}" min="1" max="365" id="days">
			</div>
			...
			<a href="/product/{{ $product->id }}/remove" class="form__remove">Remove</a>
		</form>
	</section>
</main>
@endsection